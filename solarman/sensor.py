...
class Inverter:
    ...
    def retrieve_raw_data(self):
        ...
        data = {}
        try:
            data = self._modbus.read_holding_registers(register_addr=0x20c, quantity=self._get_modbus_size())
        except IOError as e:
...
class SolarMANSensor(CoordinatorEntity, SensorEntity):
    ...
    def value(self):
        if self.raw_value:
            if self._definition.signed:
                return PySolarmanV5.twos_complement(self.raw_value, 16) * self._definition.mb_mult
